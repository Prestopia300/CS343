		...
		while (getline(file, line)) // Overview: push_back index of actor/movie
		{
			if (!line.empty())
			{
				if (line.front() != '\t') // Line starting with actor names
				{
					...
					cout << "Actor:" << actor << " and Movie: " << movie << endl;
					//if (actor != "")
					//{
				...
				else // other movies the actor was in
				{
					...
					{
						movie = current_movie;
						cout << "current movie: " << movie << endl;
						if (str.find_first_not_of(' ') != string::npos)
						{
							movies_to_actors_[movie].push_back(i); // actor index is pushed to all other movies they appear in, stored in vector<int> value of map // There's a non-space.
						}
...
bool Reader::buildGraph() // builds and displays's graph and it's bacon numbers
{
	...
	for (auto& i : movies_to_actors_)
	{
		...
		cout << "--------------------" << endl;
		cout << "Insert movie " << i.first << endl;
	...
	for (int index = 0; index < actor_index_.size(); index++)
	{
		...
		actor_graph.shortestPath(index);
